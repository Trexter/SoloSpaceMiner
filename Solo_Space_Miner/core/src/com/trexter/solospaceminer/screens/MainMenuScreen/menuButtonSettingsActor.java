 ...
 */
public class menuButtonSettingsActor extends Actor
{
    Sprite button;
    ...
    public menuButtonSettingsActor(int startX, int startY, int scaleX, int scaleY, int _deltaX, String texturePath, int _actionDelay)
    {
        menuButtonSettingsActor.this.setTouchable(Touchable.enabled);

        button = new Sprite(new Texture(texturePath));
        menuButtonSettingsActor.this.setBounds(0, 0, button.getWidth(), button.getHeight());
        ...
        addListener(new ClickListener(){
            ...
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                ...
                menuButtonSettingsActor.this.addAction(sta);

                //trigger camera movement to the play buttons next area
                MoveToAction mta = new MoveToAction();
                mta.setPosition(0, -2000);
                mta.setDuration(2f);
    ...
    protected void buttonLogic()
    {
        ...
        if(!actionAdded)
        {
            if(System.currentTimeMillis() - startTime >= actionDelay)
            {
                ...
                menuButtonSettingsActor.this.addAction(mba);
